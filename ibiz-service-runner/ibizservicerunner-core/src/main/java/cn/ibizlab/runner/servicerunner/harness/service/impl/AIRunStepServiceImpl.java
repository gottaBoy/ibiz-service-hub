...
import cn.ibizlab.runner.servicerunner.harness.service.IAIRunStepService;
...
@Service
public class AIRunStepServiceImpl extends DEServiceBase<AIRunStepDTO, SearchContextDTO> implements IAIRunStepService {

    private static final Log log = LogFactory.getLog(AIRunStepServiceImpl.class);
    ...
    private void postConstruct() throws Exception {
        ...
        if (systemGateway != null && !systemGateway.isMultiInstanceMode(SystemRuntime.class)) {
            ...
            if (service == null || service.getClass() == AIRunStepServiceImpl.class) {
        ...
        try {
            getDataEntityRuntime().registerDEActionPluginRuntime(ACTION_CREATE, new IDEActionPluginRuntime() {
                ...
                public Object execute(IDataEntityRuntimeContext context, IPSDEAction action, Object[] args, Object actionData) throws Throwable {
                    return getHarnessCoordinator().createStep(context, action, args, actionData);
                }
            });
            getDataEntityRuntime().registerDEActionPluginRuntime(ACTION_UPDATE, new IDEActionPluginRuntime() {
                @Override
                public Object execute(IDataEntityRuntimeContext context, IPSDEAction action, Object[] args, Object actionData) throws Throwable {
                    return getHarnessCoordinator().updateStep(context, action, args, actionData);
                }
            });
            getDataEntityRuntime().registerDEActionPluginRuntime(ACTION_SAVE, new IDEActionPluginRuntime() {
                @Override
                public Object execute(IDataEntityRuntimeContext context, IPSDEAction action, Object[] args, Object actionData) throws Throwable {
                    throw new UnsupportedOperationException("AI_RUN_STEP Save is not supported; use Create, Update, or append an event");
                }
            });
            getDataEntityRuntime().registerDEActionPluginRuntime(ACTION_REMOVE, passthroughAction());
            ...
            throw new RuntimeException("Failed to prepare AI_RUN_STEP service", e);
        }
    }
